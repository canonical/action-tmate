...
// EXTERNAL MODULE: ../../../../../opt/homebrew/Cellar/ncc/0.38.1/libexec/lib/node_modules/@vercel/ncc/dist/ncc/@@notfound.js?@actions/core
var _notfound_actions_core = __nccwpck_require__(574);
// EXTERNAL MODULE: ../../../../../opt/homebrew/Cellar/ncc/0.38.1/libexec/lib/node_modules/@vercel/ncc/dist/ncc/@@notfound.js?@actions/github
var github = __nccwpck_require__(194);
// EXTERNAL MODULE: ../../../../../opt/homebrew/Cellar/ncc/0.38.1/libexec/lib/node_modules/@vercel/ncc/dist/ncc/@@notfound.js?@actions/tool-cache
var tool_cache = __nccwpck_require__(389);
// EXTERNAL MODULE: ../../../../../opt/homebrew/Cellar/ncc/0.38.1/libexec/lib/node_modules/@vercel/ncc/dist/ncc/@@notfound.js?@octokit/rest
var rest = __nccwpck_require__(835);
 ...
 */
const useSudoPrefix = () => {
  const input = _notfound_actions_core.getInput("sudo");
...
const execShellCommand = (cmd, options) => {
  _notfound_actions_core.debug(`Executing shell command: [${cmd}]`)
    ...
    const proc = (external_process_default()).platform !== "win32" ?
      (0,external_child_process_namespaceObject.spawn)(cmd, [], {
        ...
        env: {
          ...
          HOMEBREW_GITHUB_API_TOKEN: _notfound_actions_core.getInput('github-token') || undefined
        }
      }) :
...
async function run() {
  try {
    ...
    if (!!_notfound_actions_core.getState('isPost')) {
      const message = _notfound_actions_core.getState('message')
      const tmate = _notfound_actions_core.getState('tmate')
      if (tmate && message) {
        const shutdown = async () => {
          _notfound_actions_core.error('Got signal')
        ...
        _notfound_actions_core.debug("Waiting")
        ...
        for (let seconds = 10 * 60; seconds > 0; ) {
          ...
          if (continueFileExists()) {
            _notfound_actions_core.info("Exiting debugging session because the continue file was created")
            break
          }

          if (didTmateQuit()) {
            _notfound_actions_core.info("Exiting debugging session 'tmate' quit")
            break
          }
    ...
    if (_notfound_actions_core.getInput("install-dependencies") !== "false") {
      _notfound_actions_core.debug("Installing dependencies")
      if ((external_process_default()).platform === "darwin") {
        ...
        _notfound_actions_core.debug("linux distro: [" + distro + "]");
        if (distro === "alpine") {
      ...
      _notfound_actions_core.debug("Installed dependencies successfully");
    }

    if ((external_process_default()).platform === "win32") {
      tmateExecutable = 'CHERE_INVOKING=1 tmate'
    } else {
      _notfound_actions_core.debug("Generating SSH keys")
      ...
      _notfound_actions_core.debug("Generated SSH-Key successfully")
    }

    let newSessionExtra = ""
    let tmateSSHDashI = ""
    let publicSSHKeysWarning = ""
    const limitAccessToActor = _notfound_actions_core.getInput("limit-access-to-actor")
    if (limitAccessToActor === "true" || limitAccessToActor === "auto") {
      const { actor, apiUrl } = github.context
      const auth = _notfound_actions_core.getInput('github-token')
    ...
    _notfound_actions_core.debug("Creating new session")
    ...
    _notfound_actions_core.debug("Created new session successfully")

    _notfound_actions_core.debug("Fetching connection strings")
    ...
    /*
      ...
      */
    _notfound_actions_core.saveState('isPost', 'true')

    const detached = _notfound_actions_core.getInput("detached")
    if (detached === "true") {
      _notfound_actions_core.debug("Entering detached mode")
      ...
      _notfound_actions_core.saveState('message', message)
      _notfound_actions_core.saveState('tmate', tmate)
      console.log(message)
      return
    }

    _notfound_actions_core.debug("Entering main loop")
    while (true) {
      if (publicSSHKeysWarning) {
        _notfound_actions_core.warning(publicSSHKeysWarning)
      }
      if (tmateWeb) {
        _notfound_actions_core.info(`Web shell: ${tmateWeb}`);
      }
      _notfound_actions_core.info(`SSH: ssh -p ${port} ${token}@${host}`);
      if (tmateSSHDashI) {
        _notfound_actions_core.info(`or: ${tmateSSH.replace(/^ssh/, tmateSSHDashI)}`)
      }
    ...
    _notfound_actions_core.setFailed(error);
  }
}
